...
function PhotosUploader({ addedPhotos, onPhotosAdded }) {
  ...
  function setMainPhotoHandler(e, link) {
    e.preventDefault();
    const addedPhotosWithourSelected = addedPhotos.filter(
      (photo) => photo !== link
    );

    const newPhotos = [link, ...addedPhotosWithourSelected];
        ...
        {addedPhotos.length > 0 &&
          addedPhotos.map((link) => (
            <div className='h-32 flex relative' key={link}>
              ...
              <button
                onClick={(e) => setMainPhotoHandler(e, link)}
